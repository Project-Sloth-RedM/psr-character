...
RegisterNetEvent("psr-character:server:saveNewCharacter", function(data)
	...
	if PSRCore.Player.Login(_source, false, PlayerData) then
		...
		TriggerClientEvent("psr-character:client:spawnPlayer", _source, skinData, clothingData, data["Sex"], true)
	end
end)

RegisterNetEvent("psr-character:server:spawnPlayer", function(citizenid, skin, clothing, sex, newPlayer)
	local _source = source
	if PSRCore.Player.Login(_source, citizenid) then
		PSRCore.ShowSuccess(GetCurrentResourceName(), GetPlayerName(_source) .. " has successfully loaded")
		-- PSRCore.Functions.RefreshCommands(_source)
		TriggerClientEvent("psr-character:client:spawnPlayer", _source, skin, clothing, sex, newPlayer)
	end
end)

RegisterNetEvent("psr-character:server:deleteCharacter", function(citizenid)
	local _source = source
	PSRCore.Player.DeleteCharacter(_source, citizenid)
	TriggerClientEvent("psr-character:client:reloadSelect", _source)
end)

AddEventHandler("PSRCore:Server:PlayerLoaded", function() end)

PSRCore.Functions.CreateCallback("psr-character:server:fetchCharacters", function(source, cb)
	local _source = source
	local identifier = PSRCore.Functions.GetIdentifier(_source, "license")
...
PSRCore.Functions.CreateCallback("psr-character:server:GetDonationTierFromLicense", function(source, cb, license)
...
PSRCore.Commands.Add("loadchar", "Load your character", {}, false, function(source)
	...
	TriggerClientEvent("psr-character:client:loadCharacter", _source, Player[1], Outfits)
end)
